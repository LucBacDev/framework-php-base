...
class Tar extends Archive
{
     ...
     * @throws ArchiveIllegalCompressionException
     */
    public function createStream($file = '')
    {
        $this->file   = $file;
        $this->memory = '';
    ...
    public function closeStream()
    {
        ...
        // write footer
        if ($this->writeaccess) {
            $this->writebytesStream(pack("a512", ""));
            $this->writebytesStream(pack("a512", ""));
        }
     ...
     * @throws ArchiveIOException
     */
    public function getArchiveStream($lastFile = true)
    {
        if($lastFile)
            $this->closeStream();
        ...
        $result = $this->memory;
        $this->memory = '';
        $this->contentLength += strlen($result);
     ...
     * @return int number of bytes written
     */
    protected function writebytesStream($data)
    {
        $this->memory .= $data;
        $written = strlen($data);
     ...
     * @throws ArchiveIOException
     */
    protected function writeFileHeaderStream(FileInfo $fileinfo)
    {
        $this->writeRawFileHeaderStream(
            $fileinfo->getPath(),
            $fileinfo->getUid(),
            $fileinfo->getGid(),
            $fileinfo->getMode(),
     ...
     * @throws ArchiveIOException
     */
    protected function writeRawFileHeaderStream($name, $uid, $gid, $perm, $size, $mtime, $typeflag = '')
    {
        // handle filename length restrictions
        $prefix  = '';
        $namelen = strlen($name);
        if ($namelen > 100) {
            ...
            if (strlen($file) > 100 || strlen($dir) > 155) {
                ...
                $this->writeRawFileHeader('././@LongLink', 0, 0, 0, $namelen, 0, 'L');
                for ($s = 0; $s < $namelen; $s += 512) {
                    $this->writebytesStream(pack("a512", substr($name, $s, 512)));
                }
                $name = substr($name, 0, 100); // cut off name
            } else {
                // we're fine when splitting, use POSIX ustar
        ...
        $this->writebytesStream($data_first);

        $chks = pack("a8", sprintf("%6s ", decoct($chks)));
        $this->writebytesStream($chks.$data_last);
    }
