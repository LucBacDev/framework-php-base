...
function convert2DefaultSql(&$zthis, $sql, $inputarr)
{
	if (is_array($inputarr) && count($inputarr) > 1) {
		unset($inputarr[0]);
		...
		foreach ($inputarr as $arr) {
			...
			foreach($arr as $v){
				...
				if ($typ == 'string') {
					...
					$sql .= $v ? $zthis->true : $zthis->false;
				} else if ($typ == 'object') {
					if (method_exists($v, '__toString')) {
						$sql .= $zthis->qstr($v->__toString());
					} else {
						$sql .= $zthis->qstr((string)$v);
					}
				} else if ($v === null) {
					$sql .= 'NULL';
				} else {
...
function _adodb_debug_execute($zthis, $sql, $inputarr)
{
	...
	if ($inBrowser) {
		if ($ss) {
			...
			$ss = '';
			$sqlTxt = convert2DefaultSql($zthis, $sql, $inputarr) . ";";
		}
		if ($zthis->debug === -1) {
			$outString = "(%s):  %s &nbsp; %s<br>\n";
			...
			$outString = "(%s):  %s  %s";
			ADOConnection::outp(sprintf($outString, $myDatabaseType, htmlspecialchars($sqlTxt, ENT_COMPAT | ENT_HTML401, 'UTF-8', false), $ss.PHP_EOL.PHP_EOL), false);
		}
	} else {
		// CLI output
		if ($zthis->debug !== -99) {
